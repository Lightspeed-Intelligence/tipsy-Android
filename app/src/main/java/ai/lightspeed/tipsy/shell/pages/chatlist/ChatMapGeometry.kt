...
internal object ChatMapGeometry {
     ...
     * ⚠️ 别把规则记成"只有带常量才有风险" —— 任何非线性/非齐次运算
     * （round / floor / clamp / 加减常量）都破坏可交换性。
     * 既然同一个文件里几类混着，就统一要求 dp 输入，
     * 参数名带 `Dp` 后缀让调用点看得见 —— 而不是逐个函数记"这个可以那个不行"。
     *
     * ⚠️ 阶段二接 `graphicsLayer` 时还要**反向**转一次：
     * `translationX/Y` 的单位是 **px**，解算结果是 dp。
     */
