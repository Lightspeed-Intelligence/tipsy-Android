# Tipsy Android 迁移工程日志（逐刀记录，append-only）

> 本文件由 `android-native-progress.md` 的 §2 于 2026-08-22 整体拆出（原文原样搬移，
> **小节编号原文保留且继续单调递增**：新刀在本文件末尾追加 `### 2.<下一号>`）。
>
> **引用规则**：代码注释 / 技能 / 记忆 / PR 里的历史引用「进度文档 §2.x」
> 一律在本文件找同号小节，编号永不复用、永不重排。
>
> **这里不是状态**：波次进度、横切能力、Surface 验收矩阵、未决问题都在
> [android-native-progress.md](android-native-progress.md)（唯一状态真值）——
> **本文中出现的 §0 / §1 / §3 / §4 / §5 / §7 也一律指该文件的对应章节**。
> 本文件是每个工作包的实测记录、踩坑与验证证据 —— 历史小节不改写，
> 后续被取代时只加「后续（§2.y）已…」指针（先例：§2.40 / §2.41 / §2.43）。

## 2. 逐刀记录（拆分前的章节名是「当前工程实况」，编号沿用）
...
| Back/去重 | `TipsyRouter.popSurfaceContainer()` 只弹当前符合身份的容器 | RN 栈底 `popSurface()`；Android 当前仍是单层已接受偏差 | Settings 容器退栈后按 `SettingsSubScreen` 类型解除 `lastHandled`；Create 沿用同纪律 | Android FragmentManager 负责 native back stack；多层 Surface 仍须先补 instanceId | `AppRouterTest` 锁同一 Settings 子屏关闭后可重开；生产 policy 仍关闭 Settings（后续 §2.48 已放行） |
...
> **后续（§2.49，2026-08-20）**：`AppRoute.EditProfile` 已进生产白名单并跑过
> 模拟器 §9.1 矩阵（换号格仍缺）。本节保留预接时的边界、威胁模型与「生产关闭」
> 的历史状态，不再更新。
...
route/Contract，**12 个业务 Surface**（注册组件 13 含 Debug）的「全启用」判据修正为
**9 启用 + 2 并入 Settings + 1 待评估（Onboarding）**。
...
它拦的类别 G1 全看不见。首跑已验：合入当天 workflow_dispatch 手动触发
...
### 2.57 W4 Screen 原生分享（2026-08-21 实现，2026-08-22 已并入 main —— PR #60）
...
URL/order/saver、审核/content、冻结 token、点击快照及推荐队列/retry 单测。
✅ **PR #60 已并入 main（2026-08-22），G1 Fast Gate 全绿** —— lint 硬门、
assembleGooglePlayDebug、release manifest、app 单测、桥单测与 skipped=0 守卫
均由 CI 组合验证（中途 `fix(screen): satisfy Android lint gate` 补了一笔 lint 修正）。
