...
⚠️ 原文「**语言设置页刻意不迁移**」是把方案 §8.1 读反了（2026-08-14 订正）——
那句指的是「不由 RN 承载」，**语言页要原生实现**，见方案 §8.1 语言页行。

⚠️ 「唯一 writer」包含 **`user-storage` 信封镜像**：壳会在 Surface 容器出栈时
从该信封读回语言并覆盖当前值，所以只写服务端会被旧值倒灌
（2026-08-18，进度文档 §2.38）。**读写方向必须成对。**
